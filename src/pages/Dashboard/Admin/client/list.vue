        ...
        >
          ...
          <template v-slot:top-left>
            <div class="items-center">
              <q-btn rounded dense no-caps label="+Add Client" color="blue-7" style="width: 130px; height:40px;" @click="createNew()" />
            </div>
          </template>
          <template v-slot:top-right>
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { name: 'depot_location', required: true, label: 'Depot Location', align: 'left', field: 'depot_location', sortable: false },
  ...
  methods: {
    ...
    getUsers: async function (props) {
      ...
      Loading.show()
      try {
        let res = await api.getClients(params)
        Loading.hide()
